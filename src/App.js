...
function App() {
  ...
  const getBotaoEstilo = (status) => ({
    ...
    border: '1px solid #ccc', borderRadius: '8px', padding: '10px 14px', margin: '3px',
  ...
  return (
    <div style={{ padding: '20px', backgroundColor: '#f0f2f5', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <style>{`
        .btn-disciplina:hover { filter: brightness(0.9); transform: translateY(-1px); box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
      `}</style>
      ...
      <header style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '15px', marginBottom: '20px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            <img src={brasao} alt="Brasão" style={{ height: '65px' }} />
            <div>
              <h1 style={{ color: '#1a73e8', margin: 0, fontSize: '24px' }}>Gestão de PEIs - EEMDP2</h1>
              <p style={{ color: '#95a5a6', margin: '5px 0 0 0', fontSize: '14px' }}>{bimestre} | ⚪ Pendente | 🟡 Correção | 🟢 Concluído</p>
            </div>
          </div>

          <div style={{ display: 'flex', gap: '10px' }}>
            <select value={bimestre} onChange={(e) => setBimestre(e.target.value)} style={{ padding: '10px', borderRadius: '10px', fontWeight: 'bold', border: '2px solid #1a73e8' }}>
        ...
        <div style={{ marginTop: '15px', display: 'flex', gap: '10px', borderTop: '1px solid #eee', paddingTop: '15px' }}>
          <button onClick={fazerBackup} style={{ backgroundColor: '#6c757d', color: '#fff', border: 'none', padding: '10px 18px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>💾 Baixar Backup</button>
          <label style={{ backgroundColor: '#17a2b8', color: '#fff', padding: '10px 18px', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold' }}>
            📤 Subir Backup <input type="file" accept=".json" onChange={importarBackup} style={{ display: 'none' }} />
          </label>
          <button onClick={copiarEEnviar} style={{ backgroundColor: '#25D366', color: '#fff', border: 'none', padding: '10px 18px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>📱 WhatsApp</button>
        </div>
      </header>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: '#fff' }}>
          <thead>
            <tr style={{ backgroundColor: '#1a73e8', color: '#fff', textAlign: 'left' }}>
              ...
              <th style={{ padding: '18px' }}>Status por Disciplina ({bimestre})</th>
            </tr>
          </thead>
          <tbody>
            {alunosParaExibir.map(aluno => (
              <tr key={aluno.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                <td style={{ padding: '15px', fontWeight: 'bold', color: '#2c3e50', minWidth: '220px' }}>{aluno.nome}</td>
                <td style={{ padding: '15px', color: '#666', fontWeight: 'bold' }}>{aluno.turma}</td>
